...
Public Class Matrix
    ...
    Public Shared Function MultiplyTranspose(A1 As Matrix, A2 As Matrix) As Matrix

        Dim rows As Integer = A1.Rows
        Dim columns As Integer = A2.Rows
        Dim K As Integer = A1.Columns
        Dim tmp As New Matrix(columns, rows)

        For i As Integer = 0 To rows - 1
            For j As Integer = 0 To columns - 1
                For _k As Integer = 0 To K - 1
                    Dim v As Double = tmp.Get(i, j)
                    v += A1.Get(i, _k) * A2.Get(j, _k)
                    tmp.Set(i, j, v)
                Next
            Next
        Next
    ...
    Public Shared Function TransposeMultiply(A1 As Matrix, A2 As Matrix) As Matrix

        Throw New NotImplementedException

        'Dim rows As Integer = A1.Columns
        'Dim columns As Integer = A2.Columns
        'Dim K As Integer = A1.Rows
        ...
        '        For _k As Integer = 0 To K - 1
        '            'Dim v As Double = tmp.Get(i, j)
        '            'v += conj(A1.Get(_k, i)) * A2.Get(_k, j)
        '            'tmp(i)(j) += conj(A1(_k)(i)) * A2(_k)(j) ' TODO
        '            'tmp.Set(i, j, v)
        '        Next
        '    Next
        'Next
